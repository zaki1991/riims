package com.hkntv.riims.core.model;

import com.hkntv.pylon.core.beans.mapping.annotation.MapClass;
import java.util.Date;

@MapClass("com.hkntv.riims.core.entity.OrDamageInfo")
public class OrDamageInfoModel{
	
	private String guid;
	private String unitId;
	private String workAreaId;
	private String lineId;
	private String lineDirection;
	private String trackId;
	private String stationId;
	private String turnoutId;
	private String alarmChannel;
	private String stationType;
	private String weldId;
	private String teamId;
	private String mileage;
	private String leftRightTrack;
	private Date uploadDate;
	private Date findTime;
	private String uploadPersonId;
	private String findPersonId;
	private String damageType;
	private String damageStatus;
	private String damageId;
	private String inspectionCarId;
	private String railId;
	private String imagePath;
	private String weldType;
	private String isTurnout;
	private String isWeldType;
	private String railType;
		
	public void setGuid(String guid){
		this.guid = guid;
	}
	
	public String getGuid(){
		return this.guid;
	}
		
	public void setUnitId(String unitId){
		this.unitId = unitId;
	}
	
	public String getUnitId(){
		return this.unitId;
	}
		
	public void setWorkAreaId(String workAreaId){
		this.workAreaId = workAreaId;
	}
	
	public String getWorkAreaId(){
		return this.workAreaId;
	}
		
	public void setLineId(String lineId){
		this.lineId = lineId;
	}
	
	public String getLineId(){
		return this.lineId;
	}
		
	public void setLineDirection(String lineDirection){
		this.lineDirection = lineDirection;
	}
	
	public String getLineDirection(){
		return this.lineDirection;
	}
		
	public void setTrackId(String trackId){
		this.trackId = trackId;
	}
	
	public String getTrackId(){
		return this.trackId;
	}
		
	public void setStationId(String stationId){
		this.stationId = stationId;
	}
	
	public String getStationId(){
		return this.stationId;
	}
		
	public void setTurnoutId(String turnoutId){
		this.turnoutId = turnoutId;
	}
	
	public String getTurnoutId(){
		return this.turnoutId;
	}
		
	public void setAlarmChannel(String alarmChannel){
		this.alarmChannel = alarmChannel;
	}
	
	public String getAlarmChannel(){
		return this.alarmChannel;
	}
		
	public void setStationType(String stationType){
		this.stationType = stationType;
	}
	
	public String getStationType(){
		return this.stationType;
	}
		
	public void setWeldId(String weldId){
		this.weldId = weldId;
	}
	
	public String getWeldId(){
		return this.weldId;
	}
		
	public void setTeamId(String teamId){
		this.teamId = teamId;
	}
	
	public String getTeamId(){
		return this.teamId;
	}
		
	public void setMileage(String mileage){
		this.mileage = mileage;
	}
	
	public String getMileage(){
		return this.mileage;
	}
		
	public void setLeftRightTrack(String leftRightTrack){
		this.leftRightTrack = leftRightTrack;
	}
	
	public String getLeftRightTrack(){
		return this.leftRightTrack;
	}
		
	public void setUploadDate(Date uploadDate){
		this.uploadDate = uploadDate;
	}
	
	public Date getUploadDate(){
		return this.uploadDate;
	}
		
	public void setFindTime(Date findTime){
		this.findTime = findTime;
	}
	
	public Date getFindTime(){
		return this.findTime;
	}
		
	public void setUploadPersonId(String uploadPersonId){
		this.uploadPersonId = uploadPersonId;
	}
	
	public String getUploadPersonId(){
		return this.uploadPersonId;
	}
		
	public void setFindPersonId(String findPersonId){
		this.findPersonId = findPersonId;
	}
	
	public String getFindPersonId(){
		return this.findPersonId;
	}
		
	public void setDamageType(String damageType){
		this.damageType = damageType;
	}
	
	public String getDamageType(){
		return this.damageType;
	}
		
	public void setDamageStatus(String damageStatus){
		this.damageStatus = damageStatus;
	}
	
	public String getDamageStatus(){
		return this.damageStatus;
	}
		
	public void setDamageId(String damageId){
		this.damageId = damageId;
	}
	
	public String getDamageId(){
		return this.damageId;
	}
		
	public void setInspectionCarId(String inspectionCarId){
		this.inspectionCarId = inspectionCarId;
	}
	
	public String getInspectionCarId(){
		return this.inspectionCarId;
	}
		
	public void setRailId(String railId){
		this.railId = railId;
	}
	
	public String getRailId(){
		return this.railId;
	}
		
	public void setImagePath(String imagePath){
		this.imagePath = imagePath;
	}
	
	public String getImagePath(){
		return this.imagePath;
	}
		
	public void setWeldType(String weldType){
		this.weldType = weldType;
	}
	
	public String getWeldType(){
		return this.weldType;
	}
		
	public void setIsTurnout(String isTurnout){
		this.isTurnout = isTurnout;
	}
	
	public String getIsTurnout(){
		return this.isTurnout;
	}
		
	public void setIsWeldType(String isWeldType){
		this.isWeldType = isWeldType;
	}
	
	public String getIsWeldType(){
		return this.isWeldType;
	}
		
	public void setRailType(String railType){
		this.railType = railType;
	}
	
	public String getRailType(){
		return this.railType;
	}
		
		
}